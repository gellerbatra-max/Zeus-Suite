"""A thin wrapper over data-platform-api's real REST API -- the only service this app talks to
per format_interchange_plan.md Sec 0 ("It never talks to Pattern Design or Marker Making
directly"). Piece geometry written by pattern-design-service is fetched here as raw Blob Storage
bytes via the platform's own download-url endpoint, parsed with this service's own mirror of the
geometry document shape (see app/geometry.py) -- the same cross-service-schema-mirroring pattern
`pattern-design-app`'s `api/types.ts` already uses against its own backend.

Identity is forwarded, not owned: every call carries the *caller's* X-Dev-User/X-Dev-Org headers
untouched, the local-dev stand-in for forwarding the operator's own Entra ID token in production.
"""
