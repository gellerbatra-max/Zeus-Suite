"""pattern-design-service has no database and no ORM models of its own -- its tests need a real
...
# them itself the same way (see marker-making-service/tests/conftest.py for the same pattern and
# the CI incident that made it necessary).
...
def platform_server():
    ...
    # output is otherwise silently dropped once nothing reads it (see marker-making-service's own
    # conftest.py / commit 6adb439 for the CI incident that made this necessary here too).
